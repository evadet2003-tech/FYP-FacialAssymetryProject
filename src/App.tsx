...
function useAuthState() {
  const [users, setUsers] = useState<User[]>(() => {
    const raw = localStorage.getItem("fas_users");
    ...
    const seeded: User[] = [
      {
        ...
        password: "12345",
        name: "System Admin",
        email: "admin@intelliface.ai",
        bio: "Platform owner",
        phone: "",
        organization: "IntelliFace",
        location: "",
        website: "",
        timezone: "UTC",
        role: "admin",
        avatarUrl: "https://images.unsplash.com/photo-1507679799987-c73779587ccf?auto=format&fit=crop&w=300&q=80",
    ...
    localStorage.setItem("fas_users", JSON.stringify(seeded));
    return seeded;
  });
  const [activeUserId, setActiveUserId] = useState<string | null>(() => localStorage.getItem("fas_active_user"));
  const user = useMemo(() => users.find((u) => u.id === activeUserId) ?? null, [users, activeUserId]);

  const signIn = (username: string, password: string) => {
    const match = users.find((u) => u.username === username && u.password === password);
    if (!match) return { ok: false as const, message: "Invalid credentials." };
    setActiveUserId(match.id);
    localStorage.setItem("fas_active_user", match.id);
    return { ok: true as const };
  };
  const signUp = (payload: Pick<User, "username" | "password" | "name" | "email">) => {
    ...
    setUsers(next);
    localStorage.setItem("fas_users", JSON.stringify(next));
    setActiveUserId(newUser.id);
    localStorage.setItem("fas_active_user", newUser.id);
    return { ok: true as const };
  };
  ...
  const signOut = () => {
    localStorage.removeItem("fas_active_user");
...
function LoginPage({ onLogin }: { onLogin: (u: string, p: string) => { ok: boolean; message?: string } }) {
...
function SignupPage({ onSignup }: { onSignup: (payload: Pick<User, "username" | "password" | "name" | "email">) => { ok: boolean; message?: string } }) {
  const navigate = useNavigate();
  ...
  const submit = (event: FormEvent) => {
    event.preventDefault();
    const result = onSignup({ name, email, username, password });
...
function NewAnalysisPage() {
  const { user, saveSession } = useOutletContext<DashboardOutletContext>();
  ...
  const saveToHistory = () => {
    ...
    if (!imageDataUrl?.trim()) {
      setError("Cannot save: no processed image. Go back through the wizard with a loaded face image.");
      return;
    }
    setError("");
    void (async () => {
      ...
      let thumb = await compressDataUrlForStorage(imageDataUrl, { maxEdge: 720, quality: 0.76 });
      let ok = saveSession({
        id: sessionId,
        userId: user.id,
        imageDataUrl: thumb,
      ...
      if (!ok) {
        thumb = await compressDataUrlForStorage(imageDataUrl, { maxEdge: 480, quality: 0.62 });
        ok = saveSession({
          id: sessionId,
          userId: user.id,
          imageDataUrl: thumb,
      ...
      if (!ok) {
        setError(
          "Still could not save: storage may be full. Clear history or free site data, then try again. Thumbnails are saved as compressed JPEGs to save space.",
        );
        return;
      }
      setStep(8);
...
function HistoryPage() {
  const { userSessions, clearAllHistory } = useOutletContext<DashboardOutletContext>();

  const onClearAll = () => {
    if (!userSessions.length) return;
    if (!window.confirm("Remove all saved sessions from this browser? This cannot be undone.")) return;
    clearAllHistory();
  };
  ...
  return (
    <section className="panel">
      ...
      <p>
        Previously uploaded images and their results are stored in this browser ({userSessions.length} session
      ...
      {!userSessions.length && (
        <p className="upload-tip">No history yet. Save a session from New Analysis (after recommendations) to store it here.</p>
      )}
      <div className="history-grid">
        {userSessions.map((session) => (
          <article key={session.id} className="history-card">
            ...
            <p className="upload-tip">Quality {session.result.quality} · Confidence {Math.round(session.result.confidence * 100)}%</p>
          </article>
        ))}
      </div>
    </section>
...
function ComparePage() {
  const { userSessions } = useOutletContext<DashboardOutletContext>();
  ...
  useEffect(() => {
    ...
    setIdA((prev) => (prev && userSessions.some((s) => s.id === prev) ? prev : userSessions[0].id));
    setIdB((prev) => {
      if (prev && userSessions.some((s) => s.id === prev)) return prev;
      return userSessions.length > 1 ? userSessions[1].id : userSessions[0].id;
    });
  }, [userSessions]);
  ...
  if (!userSessions.length) {
    return (
      <section className="panel">
        ...
        <p className="upload-tip">Save at least one session to History, then pick two runs to compare.</p>
      </section>
    );
  }
  ...
  return (
    <section className="panel">
      ...
      <div className="panel alt compare-deltas">
        ...
        <p className="upload-tip">Positive overall delta means session B scored higher (more asymmetry signal on the 0–1000 scale).</p>
      </div>
      ...
      <div className="panel alt">
        ...
        <table className="compare-table">
          ...
          <tbody>
            {barData.map((row) => {
              const d = Number((row.B - row.A).toFixed(1));
              return (
                <tr key={row.region}>
                  <td>{row.region}</td>
                  <td>{row.A}</td>
